...
-- For testing purposes, shoot action is replaced by ZombieActionShoot on all enemies (except Sniper)
-- If needed only on specific units, setup as _action_variant in the unit
-- or make a zombie charactertweakdata and use that as the unit's _tweak_table
Hooks:PostHook(CopMovement, "init", "init_zombie_test", function (self)
	if self._actions.shoot and self._actions ~= CopMovement._action_variants.team_ai and self._unit:base()._tweak_table ~= "sniper" then
		self._actions = clone(self._actions)
		self._actions.shoot = ZombieActionShoot
	end
end)
-- Tips for setting up Zombie character preset and removing their movement slowdowns:
...
-- Adds this action as an action variant
-- This allows setting up units with using "zombie" charactertweakdata
