...
-- For testing purposes, attack logic is replaced by ZombieLogicAttack on all enemies (except Cloaker and Shield)
-- If needed only on specific units, setup as _logic_variant in the unit
-- or make a zombie charactertweakdata and use that as the unit's _tweak_table
Hooks:PostHook(CopBrain, "post_init", "post_init_zombie_test", function (self)
	if self._logics.attack and self._logics.attack ~= SpoocLogicAttack and self._logics.attack ~= ShieldLogicAttack then
		self._logics = clone(self._logics)
		self._logics.attack = ZombieLogicAttack
	end
end)
...
-- This allows setting up units using "zombie" charactertweakdata
